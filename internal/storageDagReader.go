...
// A EncryptedUnixFSReader provides read-only read and seek acess to an encrypted unixfs file.
// Based off unixfs DagReader in IPFS Boxo, but each file block goes through an opening process
// and block sizes are modified to account for the AEAD nonce and overhead
type EncryptedUnixFSReader interface {
	ReadSeekCloser
	Size() uint64
	Mode() os.FileMode
	ModTime() time.Time
...
// NewEncryptedUnixFSReader creates a new reader object that reads the data represented by
// the given node, using the passed in DAGService for data retrieval.

func NewEncryptedUnixFSReader(ctx context.Context, n ipld.Node, serv ipld.NodeGetter, aead cipher.AEAD, blockAd []byte) (EncryptedUnixFSReader, error) {
	var size uint64
	...
	case *mdag.ProtoNode:
		...
		case unixfs.TFile, unixfs.TRaw:
			size = fsNode.FileSize() - uint64(fsNode.NumChildren()*(aead.NonceSize()+aead.Overhead()))
		...
		case unixfs.TMetadata:
			...
			return NewEncryptedUnixFSReader(ctx, childpb, serv, aead, blockAd)
		default:
...
func (dr *dagReader) saveNodeData(node ipld.Node) error {
	openNode, err := decryptNode(dr.ctx, node, privateStorageUnseal(dr.aead, dr.ad))
	if err != nil {
		return err
	}

	extractedNodeData, err := unixfs.ReadUnixFSNodeData(openNode)
	if err != nil {
		return err
	}
			...
			if len(node.Links()) > 0 {
				...
				for {
					childSize := fsNode.BlockSize(int(dr.dagWalker.ActiveChildIndex())) -
						uint64(dr.aead.NonceSize()+dr.aead.Overhead())
