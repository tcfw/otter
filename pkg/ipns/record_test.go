...
func TestGetOtterNode(t *testing.T) {
	sk, pk, _ := mustKeyPair(t, ic.Ed25519)
	id, _ := peer.IDFromPublicKey(pk)

	rec := mustNewRecord(t, sk, testPath, 0, time.Now().Add(time.Hour), time.Minute*10, WithOtterNodes([]peer.ID{id}))
	nodes, err := rec.OtterNodes()
	if err != nil {
		t.Fatal(err)
	}

	require.NotEmpty(t, nodes)
	require.Equal(t, id, nodes[0])
}
