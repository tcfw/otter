...
func (rec *Record) OtterNodes() ([]peer.ID, error) {
	value, err := rec.getListStringValue(cborOtterNodes)
	if err != nil {
		return nil, err
	}
...
func (rec *Record) getListStringValue(key string) ([]string, error) {
	node, err := rec.node.LookupByString(key)
	if err != nil {
		return nil, multierr.Combine(ErrInvalidRecord, err)
	}
...
func WithOtterNodes(nodes []peer.ID) Option {
	return func(o *options) {
		o.otterNodes = nodes
	}
}
...
func NewRecord(sk ic.PrivKey, value path.Path, seq uint64, eol time.Time, ttl time.Duration, opts ...Option) (*Record, error) {
	...
	node, err := createNode(value, seq, eol, ttl, options.otterNodes)
	if err != nil {
		return nil, err
	}
...
func createNode(value path.Path, seq uint64, eol time.Time, ttl time.Duration, otterNodes []peer.ID) (datamodel.Node, error) {
	...
	if len(otterNodes) != 0 {
		newOnl := basicnode.Prototype__List{}.NewBuilder()
		onl, err := newOnl.BeginList(int64(len(otterNodes)))
		if err != nil {
			return nil, err
		}
		...
		if err := onl.Finish(); err != nil {
			return nil, err
		}

		m[cborOtterNodes] = newOnl.Build()
		keys = append(keys, cborOtterNodes)
	}
